...
resource "aws_security_group" "node_group" {
  ...
  ingress {
    description     = "Allow cluster control plane to communicate with nodes (kubelet)"
    from_port       = 443
    to_port         = 443
    protocol        = "tcp"
    security_groups = [aws_security_group.cluster.id]
...
resource "aws_eks_node_group" "main" {
  ...
  tags = {
    Name = "${var.project}-${var.environment}-main-node-group"
  }
...
resource "aws_iam_role" "aws_lb_controller" {
  name = "${var.project}-${var.environment}-aws-lb-controller-role"
...
resource "aws_iam_policy" "aws_lb_controller" {
  name        = "${var.project}-${var.environment}-aws-lb-controller-policy"
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:secretsmanager:${var.aws_region}:${data.aws_caller_identity.current.account_id}:secret:gearify/${var.environment}/*"
      },
      {
        Effect = "Allow"
        Action = [
        ...
        Resource = "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.current.account_id}:parameter/gearify/${var.environment}/*"
      }
    ]
  })
}
